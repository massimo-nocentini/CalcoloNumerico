...
for cursor = 2:2:16
	...
	[x, i] = newtonMethod('function25first','function25firstDerivative', 10, 1e5, 10^(-cursor), 10^(-cursor), 10, 'incrementCriterion')
end
disp('')
disp('application of the second function')
for cursor = 2:2:16
	disp(strcat('application with tolx = ', num2str(10^(-cursor))))
	[x, i] = newtonMethod('function25second','function25secondDerivative', 10, 1e5, 10^(-cursor), 10^(-cursor), 10, 'incrementCriterion')
end
disp('------------------------------------------------------------------------')
