...
for cursor = 2:2:16
	disp(strcat('application with tolx = ', num2str(10^(-cursor))))
	[x, i] = newtonMethodAitken('function25first','function25firstDerivative', 10, 1e5, 10^(-cursor), 10^(-cursor))
end
disp('')
disp('application of the second function')
for cursor = 2:2:16
	disp(strcat('application with tolx = ', num2str(10^(-cursor))))
	[x, i] = newtonMethodAitken('function25second','function25secondDerivative', 10, 1e5, 10^(-cursor), 10^(-cursor))
end
disp('------------------------------------------------------------------------')