...
\nameref{subsec:HornerGeneralizzato}) in ascisse $\hat{x} \in linspace(-2,
2,80)$. Si nota che $\forall \hat{x} \in \mathcal{x}$ e l'interpolazione fornita
\`e soddisfacente.
\begin{center} 
\input{ApprossimazioneFunzioni/exercise47-CorrectPlotOutput.tex}
\end{center}
Nel seguente grafico invece considero ascisse $x_{\square} \in linspace(-3.005, 3.005,130)$.
Si nota che $\exists x_{\square} \not \in \mathcal{x}$: in questo caso
l'approssimazione si allontana molto dalla curva reale della $f$. Inoltre basta
allontanarsi poco dagli estremi $-3, 3$ per avere una forte differenza (non ho
riportato qui i grafici ma con $x_{\square} = -4$ si ottiene
$p_{Horner}(x_{\square}) \approx 2e^{49}$)
\begin{center} 
\input{ApprossimazioneFunzioni/exercise47-InCorrectPlotOutput.tex}
\end{center}
Riporto per completezza la curva che modella le differenze divise calcolate
sulle ascisse $x \in linspace(-3, 3,30)$:
\begin{center} 
\input{ApprossimazioneFunzioni/exercise47-DifferenzeDivisePlotOutput.tex}
\end{center}
...
che sto per descrivere, lanciare la funzione definita in
\nameref{subsec:exercise49testing}. 
\begin{center} 
\input{ApprossimazioneFunzioni/exercise49-CorrectPlotOutput.tex}
\end{center}
\begin{center} 
